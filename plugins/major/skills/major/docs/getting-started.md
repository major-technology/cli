...
This runs `pnpm install` followed by `pnpm dev`, starting a local dev server with access to connected resources via environment variables.
...
Always include `--message` to skip the interactive commit prompt. The CLI stages, commits, pushes, and deploys automatically.
