...
Always check app errors and app logs before opening Playwright. `major app errors list` and `major app logs` explain almost every server, route handler, and runtime failure without needing the browser. Run these CLI commands through `mcp__major__sandbox_bash` in the mounted app workspace; load `app-builder` if you need to mount it first. Only open the browser when the bug is purely visual, layout-related, or only observable from the rendered page.

When you do use Playwright, you are limited to **looking at the page**, not driving it:

- ✅ `mcp__major__sandbox_browser_navigate` — open a URL on `http://localhost:3000`.
- ✅ `mcp__major__sandbox_browser_take_screenshot` — capture the rendered page. Always save under `/workspace/.session-files/`; never use `/workspace/app`, repo paths, or relative paths.
- ✅ `mcp__major__sandbox_browser_snapshot` — accessibility snapshot for reading text/structure.
- ✅ `mcp__major__sandbox_browser_wait_for` — wait briefly for a pending/loading state to settle before re-screenshotting.
- ✅ `mcp__major__sandbox_browser_console_messages` — read the browser console when investigating client-side errors.
...
- Search for the route, component, handler, resource client, env var, or error text using `mcp__major__sandbox_grep` / `mcp__major__sandbox_glob`.
- Read nearby code with `mcp__major__sandbox_read_file` before editing with `mcp__major__sandbox_edit_file` or `mcp__major__sandbox_write_file`.
