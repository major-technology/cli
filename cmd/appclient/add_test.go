...
const (
	...
	otherOrgID       = "22222222-2222-4222-8222-222222222222"
	testOutsideOrgID = "cccccccc-cccc-4ccc-8ccc-cccccccccccc"
)
...
func prepare(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/applications/" + testTargetID + "/info":
			fmt.Fprintf(w, `{"applicationId":%q,"organizationId":%q,"urlSlug":"b","name":"Order Service","deployStatus":"deployed","appUrl":null}`, testTargetID, testOrgID)
		case "/applications/" + testOutsideOrgID + "/info":
			fmt.Fprintf(w, `{"applicationId":%q,"organizationId":%q,"urlSlug":"c","name":"Other Org App","deployStatus":"deployed","appUrl":null}`, testOutsideOrgID, otherOrgID)
		default:
...
func TestAddRejectsAppOutsideOrg(t *testing.T) {
	prepare(t)
	flagAddID, flagAddName, flagAddJSON = testOutsideOrgID, "", false
...
func TestAddRejectsUnauthorizedTarget(t *testing.T) {
	dir := t.TempDir()
	if err := workspace.Write(dir, workspace.Config{
		OrganizationID: testOrgID,
		Target:         workspace.Target{Kind: "app", ApplicationID: testAppID},
	...
	forbiddenID := "dddddddd-dddd-4ddd-8ddd-dddddddddddd"
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		switch r.URL.Path {
		case "/applications/" + testAppID + "/info":
			fmt.Fprintf(w, `{"applicationId":%q,"organizationId":%q,"urlSlug":"a","name":"A","deployStatus":"deployed","appUrl":null}`, testAppID, testOrgID)
		case "/applications/" + forbiddenID + "/info":
			w.WriteHeader(http.StatusForbidden)
			fmt.Fprint(w, `{"error":"not_org_builder"}`)
		default:
			t.Errorf("unexpected request: %s %s", r.Method, r.URL.Path)
			w.WriteHeader(http.StatusNotFound)
		}
	...
	flagAddID, flagAddName, flagAddJSON = forbiddenID, "", false

	if err := runAdd(&cobra.Command{}); err == nil {
		t.Fatal("expected an error for a target lookup that fails")
	}
}
